...
public class TechnicianTrackingHubService(IHubContext<TechnicianTrackingHub> hubContext, IPushNotificationSender pushNotificationSender) : ITechnicianTrackingHubService
{
    ...
    {
        await hubContext.Clients.Group(TechnicianTrackingHub.GroupName(appointmentId)).SendAsync("ReceivedTechnicianAssigned", new { appointmentId, technicianUserId }, cancellationToken);

        await pushNotificationSender.SendToUserAsync(
            customerUserId,
            "Your technician is on the way",
            "A technician has been assigned to your appointment.",
