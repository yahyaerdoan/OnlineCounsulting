...
public class HowIGetServiceManager(IMapper mapper, IGenericRepository<HowIGetService> repository, IHowIGetServiceRepository howIGetServiceRepository) : GenericService<HowIGetService, IDto>(mapper, repository), IHowIGetServiceService
{
    public async Task<IOperationResult<IQueryable<TDto>>> GetAllHowIGetServicesWithImgIconsAsync<TDto>(bool tracking = true, bool? status = true)
    {
        var entities = howIGetServiceRepository.GetAllHowIGetServicesWithImgIcons(tracking, status);
