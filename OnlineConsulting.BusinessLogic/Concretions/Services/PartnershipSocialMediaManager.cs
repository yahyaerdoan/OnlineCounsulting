...
public class PartnershipSocialMediaManager(IMapper mapper, IGenericRepository<PartnershipSocialMedia> repository, IPartnershipSocialMediRepository partnershipSocialMediaRepository) : GenericService<PartnershipSocialMedia, IDto>(mapper, repository), IPartnershipSocialMediaService
{
    public async Task<IOperationResult<IQueryable<TDto>>> GetAllSocialMediasByParnershipIdAsync<TDto>(string id, bool tracking = true, bool? status = true)
    {
        var entities = partnershipSocialMediaRepository.GetAllSocialMediasByParnershipId(id, tracking, status);

        var firstEntity = await entities.FirstOrDefaultAsync();
        var entityDisplayName = firstEntity?.EntityName ?? typeof(PartnershipSocialMedia).Name;
