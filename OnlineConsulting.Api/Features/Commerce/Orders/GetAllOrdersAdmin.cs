...
public class GetAllOrdersAdmin : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        _ = app.MapGet("/api/orders/admin", Handle)
            ...
            .WithDescription("Returns every user's orders (Super Admin only), with per-order totals and basic owner display info. Loads all users unbounded for the owner-info join, not a paged listing of its own.");
    }
