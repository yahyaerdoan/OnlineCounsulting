...
public interface ISubscriptionGateway
{
    ...
    /// <summary>Gets or creates the provider-side customer for a user. Idempotency is the caller's responsibility (CustomerMembership.ProviderCustomerId is persisted once created), so this always creates when no id is passed in from the caller.</summary>
    Task<SubscriptionCustomerResult> EnsureCustomerAsync(EnsureCustomerRequest request, CancellationToken cancellationToken = default);
    ...
    /// <summary>Attaches the given payment method to the customer as default, then starts a recurring subscription against the given price.</summary>
    Task<SubscriptionResult> CreateSubscriptionAsync(CreateSubscriptionRequest request, CancellationToken cancellationToken = default);
